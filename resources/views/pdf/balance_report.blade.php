        ...
        <tbody>
            ...
            @foreach($rows as $r)
                ...
                <tr>
                    ...
                    <td class="right">{{ number_format((float)($r->purchased_amount ?? 0),0) }}</td>
                    <td class="right">{{ number_format((int)$r->issued_qty,0) }}</td>
                    <td class="right">{{ number_format((float)($r->issued_amount ?? 0),0) }}</td>
                    <td class="right">{{ number_format((int)$r->issue_return_qty,0) }}</td>
                    <td class="right">{{ number_format((float)($r->issue_return_amount ?? 0),0) }}</td>
                    <td class="right">{{ number_format((int)$r->purchase_return_qty,0) }}</td>
                    <td class="right">{{ number_format((float)($r->purchase_return_amount ?? 0),0) }}</td>
                    <td class="right"><strong>{{ number_format((int)$r->net_balance,0) }}</strong></td>
                    <td class="right"><strong>{{ number_format((float)($r->net_amount ?? 0),0) }}</strong></td>
                </tr>
            @endforeach
            <tr class="total-row">
                <td colspan="2" class="right">Total</td>
                <td class="right">{{ number_format($tP,0) }}</td>
                <td class="right">{{ number_format($tPA,0) }}</td>
                <td class="right">{{ number_format($tI,0) }}</td>
                <td class="right">{{ number_format($tIA,0) }}</td>
                <td class="right">{{ number_format($tIR,0) }}</td>
                <td class="right">{{ number_format($tIRA,0) }}</td>
                <td class="right">{{ number_format($tPR,0) }}</td>
                <td class="right">{{ number_format($tPRA,0) }}</td>
                <td class="right">{{ number_format($tN,0) }}</td>
                <td class="right">{{ number_format($tNA,0) }}</td>
            </tr>
        </tbody>
    </table>
</body>
