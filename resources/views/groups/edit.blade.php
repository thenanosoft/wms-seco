...
<div class="max-w-3xl mx-auto">
    ...
    <form method="POST" action="{{ route('groups.update', $group) }}" class="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="text-sm font-medium">Group Code</label>
            <input name="group_code" value="{{ old('group_code', $group->group_code) }}" class="mt-1 w-full rounded-lg border-gray-200" required>
        </div>

        <div>
            <label class="text-sm font-medium">Group Name</label>
            <input name="group_name" value="{{ old('group_name', $group->group_name) }}" class="mt-1 w-full rounded-lg border-gray-200" placeholder="Optional">
        </div>
