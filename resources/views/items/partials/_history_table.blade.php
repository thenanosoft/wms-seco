        ...
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($rows as $r)
                    <tr>
                        <td class="px-4 py-2">
                            {{ \Carbon\Carbon::parse($r->txn_date)->format('d-m-Y') }}
                        </td>

                        <td class="px-4 py-2">
                            <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold
                                @if($r->txn_type === 'PURCHASE') bg-green-100 text-green-800
                                @elseif($r->txn_type === 'ISSUE') bg-red-100 text-red-800
                                @elseif(str_contains($r->txn_type,'RETURN')) bg-blue-100 text-blue-800
                                @else bg-gray-100 text-gray-800
                                @endif">
                                {{ str_replace('_',' ', $r->txn_type) }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-2 text-right">
                            {{ number_format($r->qty_in, 3) }}
                        </td>

                        <td class="px-4 py-2 text-right">
                            {{ number_format($r->qty_out, 3) }}
                        </td>
                        ...
                        <td class="px-4 py-2 text-right font-semibold">
                            {{ number_format($r->running_balance, 3) }}
                        </td>
