...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold">Edit Issue Return</h1>
            <p class="text-sm text-gray-600">Return #{{ $txn->id }} against Issue #{{ $issue->id }}</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('returns.issue.index') }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm hover:bg-gray-50">Back</a>
            <form action="{{ route('returns.issue.destroy', $txn) }}" method="POST"
                  onsubmit="return confirm('Delete this return? Stock will be reduced back from returned batches.')">
                @csrf
                @method('DELETE')
    ...
    <form method="POST" action="{{ route('returns.issue.update', $txn) }}" class="space-y-6">
        @csrf
        @method('PUT')
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 text-gray-600">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right">Issued Price</th>
                    ...
                    <tbody class="divide-y">
                        @foreach($txn->lines as $idx => $l)
                            @php
                                $issueLine = $l->issueLine;
                            @endphp
                            <tr>
                                <td class="px-4 py-2">
                                    {{ optional($issueLine?->item)->item_code }} - {{ optional($issueLine?->item)->name }}
