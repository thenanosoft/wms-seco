    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 mb-4">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
            ...
            <div>
                ...
                <select name="purchase_id" class="mt-1 w-full rounded-lg border-gray-200">
                    ...
                    @foreach($purchases as $p)
                        <option value="{{ $p->id }}" @selected(request('purchase_id')==$p->id)>{{ $p->purchase_date }} | {{ $p->reference_no ?? 'No Ref' }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('returns.purchase.index') }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium hover:bg-gray-50">Reset</a>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($rows as $r)
                    <tr>
                        ...
                        <td class="px-3 py-2">{{ $r->purchase_date }} <div class="text-xs text-gray-500">{{ $r->reference_no }}</div></td>
                        ...
                        <td class="px-3 py-2 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="{{ route('returns.purchase.edit', $r->purchase_return_transaction_id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-50" title="Edit">
                                ...
                                <form action="{{ route('returns.purchase.destroy', $r->purchase_return_transaction_id) }}" method="POST" onsubmit="return confirm('Delete this return? Stock will be restored back.')">
                                    @csrf
                                    @method('DELETE')
